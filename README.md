...
`React` `Next.js` `Node.js` `Express` `Tailwind CSS` `D3` `Pandas` `NumPy` `Scikit-learn` `SciPy` `Matplotlib` `Seaborn` `Pygame` `JUnit`

### 🔧 Tools & Platforms
`Git` `GitHub` `VS Code` `IntelliJ` `Jupyter Notebook` `MySQL` `Figma` `Vercel` `Auth0` `Raspberry Pi` `Google Apps Script`

### ⚙️ Methodologies
`Agile Development` `CI/CD` `RESTful APIs` `Full-Stack Development`
